    ...
    with gr.Accordion(label="Motor Positions"):
        ...
        with gr.Row():
            with gr.Column(min_width=1300):
                ...
                start_btn.click(fn=start_movement, inputs=[*mot_deg, *mot_speed, *mot_inverse, *mot_mult, *mot_accel, *mot_reduc, global_mot_speed, global_mot_accel])
            home_btn = gr.Button("Set All to Position 0")
            zero_state = gr.State(0)
            home_btn.click(start_movement, inputs=[*[zero_state for j in range(6)], *mot_speed, *mot_inverse, *mot_mult, *mot_accel, *mot_reduc, global_mot_speed, global_mot_accel])
