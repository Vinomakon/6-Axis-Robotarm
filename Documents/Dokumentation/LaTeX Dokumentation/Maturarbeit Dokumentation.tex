...
\lstinputlisting[language=cppp, caption={for-Loop für das Überprüfen aller laufenden Motoren und deren Anhaltung\label{lst:cpp_stop_move_for}}]{code/cpp/stop_move.cpp.txt}
\newpage
...
\noindent Durch das die Endposition des Endes vom ersten Glied bekannt ist, wird diese eigesetzt, um die Endposition des Endes vom zweiten Glied herauszufinden:
...
\centering{
\begin{multicols}{3}
\begin{tikzpicture}[tdplot_main_coords, scale=1.5] % X-Rotation
  % axes
  \draw[->, Xcol] (0,0,0) -- (1.4,0,0) node[below left] {\textcolor{Xcol}{$x$}};
...
\begin{tikzpicture}[tdplot_main_coords, scale=1.5] % Y-Rotation
  % axes
  \draw[->] (0,0,0) -- (1.4,0,0) node[below left] {\textcolor{Xcol}{$x$}};
  \draw[->, Ycol] (0,0,0) -- (0,1.4,0) node[right]  {\textcolor{Ycol}{$y$}};
  \draw[->] (0,0,0) -- (0,0,1.4) node[above]        {\textcolor{Zcol}{$z$}};
...
\begin{tikzpicture}[tdplot_main_coords, scale=1.5] % Z-Rotation
  % axes
  \draw[->] (0,0,0) -- (1.4,0,0) node[below left] {\textcolor{Xcol}{$x$}};
  \draw[->] (0,0,0) -- (0,1.4,0) node[right]  {\textcolor{Ycol}{$y$}};
  \draw[->, Zcol] (0,0,0) -- (0,0,1.4) node[above]  {\textcolor{Zcol}{$z$}};
...
\noindent Jede von diesen Rotationsachsen verfügt über einer eigenen Winkelrotation.
\begin{center}
\begin{tabular}{c c c}
    $X$-Rotation: $\varphi$ &\hspace{2.5cm} $Y$-Rotation: $\theta$ &\hspace{2.5cm} $Z$-Rotation: $\psi$
\end{tabular}
\end{center}
...
\[\theta=\atan2\left(y_D,x_D\right)={tan}^{-1}{\left(\frac{y_D}{x_D}\right)}\]
...
\end{align*}
\begin{OutBox}
Es gibt eine einzige Ausnahme in dieser Situation, die als eine Singularität definiert wird. Diese entsteht, wenn $r_{11}=1$ entspricht. Da
...
Dadurch können die Gelenkwinkel durch die vordefinierten Berechnungen erhalten werden:
